...
test.describe("Deal Master Game Flow", () => {
  ...
  test("should allow final reveal when rejecting all deals", async ({
    page,
  }) => {
    // Start the demo
    await page.click("text=Start Demo Game");
  ...
  test("should show correct case states", async ({ page }) => {
    ...
    for (let i = 0; i < 5; i++) {
      await expect(page.locator(`[data-testid="case-${i}"]`)).toContainText(
        `Case ${i + 1}`
      );
    }
    ...
    // Choose a case
    await page.click('[data-testid="case-0"]');
    await expect(page.locator('[data-testid="case-0"]')).toContainText(
      "YOUR CASE"
    );
